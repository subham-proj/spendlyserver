...
async function fetchGoogleUserInfo(accessToken: string): Promise<{ name: string | null; picture: string | null }> {
  try {
...
export const oAuthHandler = asyncHandler(async (req: Request, res: Response) => {
  ...
  try {
    ...
    const user = await User.findOneAndUpdate(
      { email },
      {
        ...
        refreshTokenExpiresIn: (tokens as any).refresh_token_expires_in ?? null,
    ...
    try {
      ...
      console.warn(`[OAuth] gmail.users.watch failed for ${email}:`, (watchErr as Error).message);
    }
...
export const getMobileAuthUrl = asyncHandler(async (req: Request, res: Response) => {
...
export const mobileCallbackHandler = asyncHandler(async (req: Request, res: Response) => {
  const { code } = req.query;
  ...
  try {
    ...
    try {
      ...
      console.warn(`[Mobile OAuth] gmail.users.watch failed for ${email}:`, (watchErr as Error).message);
    }
