...
export const getCategoryExpenses = asyncHandler(async (req, res) => {
    const userId = new mongoose.Types.ObjectId(req.userId);
    const period = req.query.period ?? "month";
    const now = new Date();
    const startOfMonth = new Date(now.getFullYear(), now.getMonth(), 1);
    const dateFilter = period === "month" ? { emailDate: { $gte: startOfMonth, $lte: now } } : {};
...
export const getAIInsights = asyncHandler(async (req, res) => {
    ...
    // Process summary
    const [facet] = summaryResult;
    const totalExpense = facet?.expense?.[0]?.total ?? 0;
    const totalIncome = facet?.income?.[0]?.total ?? 0;
    const savingsRate = totalIncome > 0 ? ((totalIncome - totalExpense) / totalIncome) * 100 : 0;
    // Process categories
    const catTotal = categoryRows.reduce((s, r) => s + r.amount, 0);
    const topCats = categoryRows.map((r) => ({
        category: r._id,
    ...
    try {
        const completion = await getGroq().chat.completions.create({
            ...
            messages: [
                {
                    role: "system",
                    content: 'You are a personal finance assistant. Always respond with a JSON object containing an "insights" array.',
                },
                {
                    role: "user",
...
function buildFallbackInsights(totalExpense, totalIncome, savingsRate, topCats) {
    const insights = [];
    if (topCats.length > 0) {
        const top = topCats[0];
        insights.push({
