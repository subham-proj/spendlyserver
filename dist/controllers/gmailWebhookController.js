...
export const gmailWebhookHandler = asyncHandler(async (req, res) => {
    ...
    const userOAuth2Client = new google.auth.OAuth2(process.env.CLIENT_ID, process.env.CLIENT_SECRET, `http://localhost:${process.env.PORT || 3000}/auth/callback`);
    ...
    try {
        ...
        console.warn(`[GmailWebhook] history.list failed for ${emailAddress}:`, err.message);
    ...
    for (const record of historyRecords) {
        ...
        for (const added of addedMessages) {
            ...
            try {
                ...
                console.warn(`[GmailWebhook] messages.get failed for ${messageId}:`, msgErr.message);
                continue;
            }
